...
- [ ] **章ヘッダーの重複問題**
  - 現状: エクスポート時に `## 第N章: タイトル` を追加するが、Scene Layer の出力にも `## 第N章 ...` が含まれることがあり、二重ヘッダーになる。
  - 原因: LLM が `# 本文` フォーマットを守らず、章ヘッダーを本文に含めてしまう。
  - 対策案:
    1. プロンプトで「章ヘッダーは出力しないこと」をより明確に指示
    2. パース時に章ヘッダー (`## 第.*章`) を除去する後処理を追加
    3. エクスポート側で章ヘッダーを追加しない（シーン本文に任せる）
  - 現状は「エクスポート側で追加」を維持（LLM が出さなかった場合の保険として）
